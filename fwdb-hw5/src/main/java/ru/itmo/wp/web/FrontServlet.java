...
public class FrontServlet extends HttpServlet {
    ...
    private void process(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            System.out.println("came to processing");
    ...
    private void process(Route route,
        ...
        for (Class<?> clazz = pageClass; afterMethod == null && clazz != null; clazz = clazz.getSuperclass()) {
            try {
                afterMethod = clazz.getDeclaredMethod("after", HttpServletRequest.class, Map.class);
            } catch (NoSuchMethodException ignored) {
                // No operations.
            }
        }
        ...
        for (Class<?> clazz = pageClass; method == null && clazz != null; clazz = clazz.getSuperclass()) {
            try {
                methods = pageClass.getDeclaredMethods();
        ...
        try {
            ...
            for (int i = 0; i < arguments.length; i++) {
                if (arguments[i].equals(Map.class)) {
                    args[i] = view;
                }
                if (arguments[i].equals(HttpServletRequest.class)) {
                    args[i] = request;
                }
            }
