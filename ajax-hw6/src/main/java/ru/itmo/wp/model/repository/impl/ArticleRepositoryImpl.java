...
public class ArticleRepositoryImpl implements ArticleRepository {
        ...
        for (int i = 1; i <= metaData.getColumnCount(); i++) {
            switch (metaData.getColumnName(i)) {
                ...
                case "creationTime":
                    ...
                    break;
                case "typeArticle":
                    article.setShown(resultSet.getString(i).equals("SHOW"));
                default:
                    // No operations.
            }
        }
    ...
    @Override
    public void setHidden(long id, String val) {
        System.out.println("came to setHidden");
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement(
                    "UPDATE Article SET typeArticle=? WHERE id=?")) {
                statement.setString(1, val);
                statement.setLong(2, id);
                if (statement.executeUpdate() != 1) {
                    System.out.println("gg");
                    throw new RepositoryException("Can't update article type");
                }
            }
        } catch (SQLException e) {
    ...
    public void save(Article article) {
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement("INSERT INTO `Article` (`userId`, `title`, `text`, `typeArticle`, `creationTime`) VALUES (?, ?, ?, ?, NOW())", Statement.RETURN_GENERATED_KEYS)) {
